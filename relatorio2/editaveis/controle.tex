...
\begin{table}[htbp]
	...
	\begin{tabular}{|p{2em}|p{6.285em}|p{7.215em}|p{6.5em}|}
		...
		\midrule
		\rowcolor[rgb]{ .851,  .851,  .851} R02   & Integrantes não se comunicam devidamente & Incentivo maior à comunicação, mudança de meio de comunicação. & Média \\
		\midrule
		R03   & Desinteresse de parte da equipe & Conversar para tentar recuperar esses membros. Em pior caso, reportar ao professor. & Alta \\
		\midrule
		\rowcolor[rgb]{ .851,  .851,  .851} R04   & Dificuldade de combinar horários de todos os integrantes do grupo & Reuniões mais frequentes entre os membros de um setor, e entre os líderes de cada setor. & Média \\
		\midrule
		R05   & Medição com valores muito diferentes do esperado, ou a não medição da água no reservatório & Verificar via sensor de fluxo se está ocorrendo alguma medição não-esperada. Se sim, parar o teste e substituir o sensor. & Baixa \\
		\midrule
		\rowcolor[rgb]{ .851,  .851,  .851} R06   & Medição com valores muito diferentes do esperado, ou a não medição da temperatura no sensor ST1 & Verificar a temperatura medida em ST3. Caso esteja muito acima do esperado interromper o teste. Trocar o sensor, caso necessário. & Baixa \\		
		\bottomrule
	\end{tabular}
\end{table}
\newpage
