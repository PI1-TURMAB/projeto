...
\subsection{Controle de Resposta:}
...
\subsubsection{Placa de Processamento}
...
\begin{table}[!htbp]
	...
	\begin{minipage}{0.88\textwidth}
		\begin{tabular}{|p{4em}p{8em}p{6em}cc|}
			\toprule
			\rowcolor[rgb]{ .267,  .447,  .769} \textcolor[rgb]{ 1,  1,  1}{\textbf{Tipo}} & \textcolor[rgb]{ 1,  1,  1}{\textbf{Nome}} & \textcolor[rgb]{ 1,  1,  1}{\textbf{Quantidade (unidades)}} & \textcolor[rgb]{ 1,  1,  1}{\textbf{Preço}} & \textcolor[rgb]{ 1,  1,  1}{\textbf{ Total }} \\
			\midrule
			\rowcolor[rgb]{ .851,  .882,  .949} Sensor & Sensor de Temperatura para o adaptador da câmara de combustão & 1     & R\$ 382,00 &  R\$  382,00  \\
			\midrule
			Sensor & Sensor de Temperatura para a tubulação & 1     & R\$ 16,80 &  R\$    16,80  \\
			\midrule
			\rowcolor[rgb]{ .851,  .882,  .949} Sensor & Sensor de Temperatura para o reservatório & 1     & R\$ 16,80 &  R\$    16,80  \\
			\midrule
			Sensor & Sensor de Pressão para a tubulação & 1     & R\$ 55,00 &  R\$    55,00  \\
			\midrule
			\rowcolor[rgb]{ .851,  .882,  .949} Sensor & Sensor de Fluxo para a tubulação & 1     & R\$ 28,80 &  R\$    28,80  \\
			\midrule
			Sensor & Sensor de Nível para o reservatório & 1     & R\$ 24,90 &  R\$    24,90  \\
			\midrule
			\rowcolor[rgb]{ .851,  .882,  .949} Placa de Processamento & Placa Arduíno Uno Rev.3  & 1     & R\$ 47,90 &  R\$    47,90  \\
			\midrule
			Módulo  & Módulo relé com 2 canais (Bombas d'água) & 1     & R\$ 12,90 &  R\$    12,90  \\
			\midrule
			\rowcolor[rgb]{ .851,  .882,  .949} Módulo  & Módulo relé com 4 canais (Ventoinhas e Válvulas hidráulicas) & 2     & R\$ 24,90 &  R\$    49,80  \\
			\midrule
			\textbf{Total} & -     & -     & -     &  R\$  634,90  \\
			\bottomrule
		\end{tabular}
	\caption{Orçamento dos materiais de controle}
	\end{minipage}
	
\end{table}\\

\newpage
\subsection{Riscos da equipe de controle}

\begin{table}[h]
	\centering
	\begin{tabular}{|p{1.5em}|p{4.2em}|p{5em}|p{8.3em}|p{6em}|p{3.8em}|p{4.2em}|}
		\toprule
		\textbf{ID} & \textbf{Risco} & \textbf{Tipo do Risco} & \textbf{Descrição} & \textbf{Descrição do Impacto} & \textbf{Impacto} & \textbf{Probabi- lidade} \\
		\midrule
		\rowcolor[rgb]{ .851,  .851,  .851} R01   & Falha no sensor de nível no reservatório & \textcolor[rgb]{ .141,  .161,  .18}{Técnico} & Medição com valores muito diferentes do esperado ou a não medição & Causa uma estimativa errônea sobre o volume de água no reservatório & Médio & Muito Baixa \\
		\midrule
		R02   & Falha no sensor de temperatura do reservatório(ST1) & \textcolor[rgb]{ .141,  .161,  .18}{Técnico} & Medição com valores muito diferentes do esperado ou a não medição & Ocasiona uma resposta errônea do sistema de arrefecimento & Médio &  Muito Baixa \\
		\bottomrule
	\end{tabular}
...
\begin{table}[h]
	\centering
	\begin{tabular}{|p{1.5cm}|p{4cm}|p{4cm}|p{2cm}|}
		\toprule
		\textbf{ID} & \textbf{Descrição} & \textbf{Ação} & \textbf{Prioridade} \\
		\midrule
		R01   & Medição com valores muito diferentes do esperado, ou a não medição da água no reservatório & Verificar via sensor de fluxo se está ocorrendo alguma medição não-esperada. Se sim, parar o teste e substituir o sensor. & Baixa \\
		\midrule
		\rowcolor[rgb]{ .851,  .851,  .851} R02   & Medição com valores muito diferentes do esperado, ou a não medição da temperatura no sensor ST1 & Verificar a temperatura medida em ST3. Caso esteja muito acima do esperado interromper o teste. Trocar o sensor, caso necessário. & Baixa \\		
		\bottomrule
	\end{tabular}
	\caption{Tabela de Ações mitigatórias e preventivas para os riscos apontados}
\end{table}
\newpage
